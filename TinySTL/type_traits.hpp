...
template <typename T> constexpr T* addressof(T& val) noexcept {
...
namespace {
template <typename T, typename = void> struct AddReference {
    using Lvalue = T;
    using Rvalue = T;
};

template <typename T> struct AddReference<T, void_t<T&>> {
    using Lvalue = T&;
    using Rvalue = T&&;
};

template <typename T, bool is_function_type = false> struct AddPointer {
    using type = typename remove_reference<T>::type*;
};

template <typename T> struct AddPointer<T, true> { using type = T; };

template <typename T, typename... Args> struct AddPointer<T(Args...), true> {
    using type = T (*)(Args...);
};
...
template <typename T> struct is_enum : bool_constant<std::is_enum<T>::value> {};
...
template <typename T> struct IsFloatingPoint : false_type {};
...
template <typename T> struct IsIntegral : false_type {};

template <> struct IsIntegral<bool> : true_type {};

template <> struct IsIntegral<char> : true_type {};

template <> struct IsIntegral<unsigned char> : true_type {};

template <> struct IsIntegral<signed char> : true_type {};
...
template <typename T> struct is_pod : bool_constant<std::is_pod<T>::value> {};
...
template <typename T> struct IsCharacter : false_type {};

template <> struct IsCharacter<char> : true_type {};

template <> struct IsCharacter<signed char> : true_type {};

template <> struct IsCharacter<unsigned char> : true_type {};

template <typename T> struct IsMemberPointer : false_type {};

template <typename T, typename C> struct IsMemberPointer<T C::*> : true_type {};
...
template <typename T> struct UnRefWrapHelper {
    using type = T;
    static constexpr bool isRefWrap = false;
};

template <typename T> struct UnRefWrapHelper<reference_wrapper<T>> {
    using type = T&;
    static constexpr bool isRefWrap = true;
};

template <typename T> struct UnRefWrap {
...
template <typename Traits> constexpr bool negation_v = negation<Traits>::value;

// map
template <typename T, bool isMap> struct AssociatedTypeHelper {
    using key_type = typename T::first_type;
    using mapped_type = typename T::second_type;
};

// set
template <typename T> struct AssociatedTypeHelper<T, false> {
    using key_type = T;
