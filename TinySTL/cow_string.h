...
private:
    ...
    void _Resize(size_type count, Lambda lambda)
    {
        ...
        else if (count > value->size)   // count > old size
        {
...
private:
    ...
    size_type _Find(const CharT* s, size_type pos, size_type count) const
    {
        ...
        {
            if (traits_type::eq(value->data[i], s[0])) // matched first character
            {
                size_type j;
                for (j = 1; j < count; ++j) 
                {
...
public:
    ...
    {
        const CharT* find_at = traits_type::find(value->data + pos, size() - pos, ch);
        return find_at == nullptr ? npos 
